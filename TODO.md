...
- [ ] **Plan 079b (or follow-up)** — Org-admin parallel bug (GLM 5.1 plan-079 flag). `src/app/(portal)/org/units/page.tsx:172` has the same `/teacher/units/{id}/edit` link; an org-admin without teacher role gets bounced. Add `/org/units/{id}` read-only detail mirroring plan 079.
