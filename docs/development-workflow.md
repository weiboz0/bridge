...
Do NOT open a PR after each phase. After Step 4 (Verify), the 4-way code review (Step 5) runs against the consolidated branch diff. The PR is opened in Step 6, after all reviewers concur.
...
Code review catches what self-review misses. The 4-way review fires **once per plan** against the consolidated branch diff, after Step 4 (Verify) and before the PR is opened. Findings go in the plan file's `## Code Review` section. Not after each phase.

1. Request code review (4-way: self on Opus, Codex, DeepSeek V4 Flash, GLM 5.1 — see `docs/reviewers.md`). Reviewers append findings to the plan file's `## Code Review` section, following `docs/code-review.md` format.
