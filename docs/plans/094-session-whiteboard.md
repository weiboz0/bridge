...
- *Single-instance topology — required and checked.*
  ...
  Every service generates a random `instance` id once at process start and returns it with each attestation.
  The verifier samples each origin five times on fresh connections (`Connection: close`, no keep-alive reuse) and fails with the class *multiple instances* unless the id is constant per origin; the gate exports the three ids, and `seed.setup.ts` fails unless its own samples return the same three.
...
- *Opt-in and non-disclosure.* `BRIDGE_E2E_STACK=1` (exactly `1`) exposes the attestation.
  ...
  Next.js has no start-time exposure signal; its protection is the uniform 404 and the fact that a request without the gate's live lock does no more than one bounded read.
...
- *Gate.* New `scripts/check-e2e-stack.mjs` exports `verifyE2EStack()` and is called by `run_e2e_gate` after the URL is resolved and **before** the demo-seed restore and Playwright.
  ...
  On failure it prints, **per service**, the failure class — not attested, mismatch, multiple instances, unreachable, redirect, or timeout — plus a remediation line (“start all three services with `BRIDGE_E2E_STACK=1` against the gate's `_test` database, or point `E2E_BASE_URL` at such a stack”; for Hocuspocus it also prints the realtime origin it derived and that `NEXT_PUBLIC_HOCUSPOCUS_URL` is baked at build time), records a named gate failure, removes a stale attestation, and returns before the seed or Playwright run.
  ...
  The real enforcement seams are `ci-local.sh` itself (which writes or deletes the file), the `pre-push` hook, the `br-autopilot` merge guard, and `docs/development-workflow.md` Step 6.8 — none of which change in this phase.
