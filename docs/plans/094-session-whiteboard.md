...
- `[OPEN]` `[sol][IMPORTANT]` `server/hocuspocus.canvas.test.ts:695-710` invokes the registered canvas admission hook with a real Hocuspocus 3.4.4 nested sync envelope and requires the lifecycle to receive the decoded update, not the outer frame.
- `[OPEN]` `[sol][IMPORTANT]` `server/hocuspocus.canvas.test.ts:713-725` and `server/canvas-lifecycle.test.ts:309-317` require authorization to be lifecycle-owned after the eight-slot turnstile, with the authenticated `userId` and its owned `AbortSignal`; the current pre-cap fetch and lifecycle callback omit that ownership.
- `[OPEN]` `[sol][IMPORTANT]` `server/canvas-lifecycle.test.ts:320-327` and `server/hocuspocus.canvas.test.ts:728-739` cover the cold-load ownership boundary: `afterLoadDocument` runs before pinned registry insertion, a synchronously inserted exact generation survives its next-turn watchdog, while an unclaimed generation must release on that turn rather than survive a logical load absent claim, unload, or destroy.
- `[OPEN]` `[sol][IMPORTANT]` `server/hocuspocus.canvas.test.ts:741-751` requires the direct hook to expose stable `session_freezing` as its structured reason, which the installed Hocuspocus `Connection` close path propagates as the websocket CLOSE reason.
- `[OPEN]` `[sol][IMPORTANT]` `tests/unit/use-yjs-provider.test.ts:79-96` passes the installed provider an `ArrayBuffer` CLOSE frame and requires forced token remint and reconnect without emitting an unauthenticated queued write.
- `[OPEN]` `[sol][IMPORTANT]` `server/canvas-lifecycle.test.ts:368-380` requires entry-by-entry incremental streaming; it rejects one aggregate JSON response containing both snapshot identities.
- `[OPEN]` `[sol][IMPORTANT]` `server/canvas-lifecycle.test.ts:330-340` requires capture accounting to shrink from the conservative reservation to exact immutable bytes before registered canvas connections close.
- `[OPEN]` `[sol][IMPORTANT]` `server/canvas-lifecycle.test.ts:342-354` requires same-connection disconnect cancellation to await the abort's authorization settlement before release.
- `[OPEN]` `[sol][IMPORTANT]` `server/canvas-lifecycle.test.ts:356-365` floods a matching terminal request and requires every caller to await one coalesced cleanup promise rather than enqueue independent serializer work.
- `[OPEN]` `[sol][IMPORTANT]` `server/hocuspocus.canvas.test.ts:755-769` exercises both installed bearer authorization paths and requires `redirect: "error"` before either bearer is sent.
- `[OPEN]` `[sol][IMPORTANT]` The above contracts deliberately use real installed Hocuspocus envelopes/hooks, `Document` mutex/connection behavior, and the actual provider message entry point instead of façade-only mocks; the cold-load watchdog, unload/destroy boundary, and all relevant production matrices remain explicit RED requirements.
...
- `[ADDRESSED]` `[sol]` Registered Hocuspocus admissions decode the nested installed sync envelope and pass decoded bytes, authenticated user identity, and the lifecycle-owned 500ms abortable authorization into the eight-slot turnstile; pre-turnstile mutation authorization is removed. Reviewer confirmation remains pending.
- `[ADDRESSED]` `[sol]` Lifecycle load uses a temporary reserved Y.Doc, an exact after-load registry claim, generation watchdogs, actual unload cancellation, destroy-only release, unique per-frame admission identities, coalesced terminal promises, and awaitable connection-owned cancellation. Reviewer confirmation remains pending.
- `[ADDRESSED]` `[sol]` Capture reserves before encode, shrinks atomically to exact incremental-response bytes before socket closure, streams entry-by-entry with registered writer deadlines and backpressure, and preserves empty-list no-allocation behavior. Reviewer confirmation remains pending.
- `[ADDRESSED]` `[sol]` Both bearer authorization fetch paths refuse redirects; temporary freeze errors expose the stable reason; the installed browser bridge handles `ArrayBuffer` CLOSE frames, fences outgoing writes during remint, then restores token/sendToken/sync without an unauthenticated replay. Reviewer confirmation remains pending.

### Phase 10 — Sol Round 3 remediation (2026-08-12; Terra; reviewer confirmation pending)

- `[OPEN]` `[sol]` Transactional after-load rollback, terminal-promise promotion, exact Yjs transaction-origin correlation, bounded completion tombstones, retryable capture CLOSEs, and abort-owned control streaming have been amended with installed-path regressions. The Round 3 review remains open pending Sol's review of this substantive commit.
...
- `[OPEN]` `[sol]` These changes are addressed by the named installed-path tests below but remain open until Sol reviews this exact commit.
...
- `[ADDRESSED]` `[sol]` The production load adapter transactionally rolls back both claim initialization failure and a later registered after-load extension failure before Hocuspocus registry insertion; a later same-name generation survives stale callbacks.
...
- `[OPEN]` `[sol]` All Round 4/5 findings remain open until Sol reviews the exact remediation commit; no author-side statement in this section is reviewer confirmation.
- `[GREEN evidence]` With both database URLs pinned to `postgresql://work@127.0.0.1:5432/bridge_test`, the database guard passed and `bun test server/hocuspocus.canvas.test.ts server/canvas-lifecycle.test.ts` passed 82 tests / 245 assertions.
