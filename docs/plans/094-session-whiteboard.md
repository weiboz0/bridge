...
**Status:** Revision 4 — folds plan-review round 3 (GLM APPROVE-WITH-NITS; Codex/Opus mechanical). Awaiting round-4 confirmation.
...
**Ended = no writes, enforced server-side (R2 blockers 1–2; R3 hardening).** Mint alone is insufficient (a 25-min write token outlives `ended`), and `onStoreDocument` alone is insufficient too (R3 — Hocuspocus applies a writable connection's update in-memory and broadcasts it to peers *before* storage, so gating storage stops persistence but not the live relay). So all of: (a) on the session `ended` transition, **force-close (or flip `readOnly` on) the canvas `documentName` connections** so an already-connected writer stops relaying; (b) `onStoreDocument` drops updates when `ended` (durable backstop); (c) every mutating canvas endpoint 409s when `ended`; (d) mint issues only `readOnly=true` for ended sessions. If the transition-time force-close proves impractical in Hocuspocus 3.4.4, the fallback is to **accept a bounded transient live-relay window explicitly as an MVP note** — durable archive is still correct — rather than imply `onStoreDocument` closes it.
...
### Round 4 — pending confirmation of Revision 4.
