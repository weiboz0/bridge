...
11. **Self-target guards in 3 places:** suspend (handler 400), demote-from-admin (handler 400), AND the UI's `isSelf={true}` branch in `UserActions` that hides destructive items but keeps View details (per Decision #18). Defense in depth. The page-level row check at `page.tsx:84` no longer hides the component entirely — it passes `isSelf` as a prop instead.
...
| Self-suspend / self-demote: admin locks themselves out | high | Decision #11 — self-target guards at handler (400 on self-suspend / self-demote) AND UI (`<UserActions isSelf={true} />` renders only View details, hides Login as / Suspend / Toggle platform-admin per Decision #18). |
| NextAuth `authorize` callback doesn't check `users.status` (Kimi K2.6 round-2) | low | A suspended user can still complete the NextAuth email/password (or OAuth) sign-in flow — `src/lib/auth.ts` `authorize` queries Drizzle, verifies password, returns user without inspecting `status`. They get a valid NextAuth session, then 401 on the first subsequent request to a Go-proxied endpoint. Functionally safe (they can't do anything once they hit any Go endpoint, which is everything that matters), but jarring UX. Acceptable v1; a follow-up plan can add a `status` check to `authorize` for graceful "Account suspended" sign-in failure. Not a security gap — the RequireAuth gate at the Go layer is authoritative. |
