...
func newBookFixture(t *testing.T) *bookFixture {
	...
	admin := insertFixtureUser(t, db, store.RegisterInput{Name: "Admin", Email: "book-admin-" + t.Name() + "@example.com", Password: "testpassword123"})
	other := insertFixtureUser(t, db, store.RegisterInput{Name: "Other", Email: "book-other-" + t.Name() + "@example.com", Password: "testpassword123"})
